...
def extract_bundle_emb(words, model=None, emb_dir=None):
    global INFERSENT_MODEL 
    if not model:
        if not INFERSENT_MODEL:
            print(SETTING)
            print(SETTING['infersent'])
            INFERSENT_MODEL = load_infersent_model(SETTING['infersent'])
        model = INFERSENT_MODEL

# [ TODO ] CURRENTLY NOT FINISHED!!! #
def get_bundle_emb(bundle, infersent_model=None, w2v_model=None, emb_dir=None):
    if emb_dir:
        path = check_emb_exists(emb_dir, bundle)
        if path:
            return np.load(path)
    # extract
    words = bundle.split(' ')
    words_emb = get_all_word_emb(words, model=w2v_model, emb_dir=emb_dir)

    # get bos and eos
    emb_bos = check_emb_exists(emb_dir, bos)
    emb_eos = check_emb_exists(emb_dir, eos)
    if not (emb_bos and emb_eos):
        LOG('error', 'eos or bos embedding not found')
        return None
...
if __name__ == '__main__':
    def parse_args():
        ...
        parser.add_argument('-v', '--verbose', type=int, help="verbose level. 0=only error, 3=debugging")
